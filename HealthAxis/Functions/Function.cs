...
namespace HealthAxis.Functions
{
    ...
    public class Function
    {
        ...
        private static Patient.Genders ReadOptionalGender(Patient.Genders current)
        {
            ...
            if (Enum.TryParse(input, true, out Patient.Genders gender) && Enum.IsDefined<Patient.Genders>(gender)) return gender;
        ...
        private static Doctor.Specialisations ReadOptionalSpecialisation(Doctor.Specialisations current)
        {
        ...
        private static Doctor? ChooseDoctorFromList(List<Doctor> doctors)
        {
            while (true)
            {
                ...
                if (!selectedDoctor.IsPractising)
                {
                    return null;
                }
        ...
        public void AddDoctor()
        {
            try
            {
                ...
                doctor.Experience = ReadIntWithRetry("Enter Years of Experience: ", "Enter a valid years of experience.");
                if (doctor.Experience < 0 || doctor.Experience > 50)
                {
                    throw new ArgumentException("Enter a valid years of experience.");
                }

                doctor.Fees = ReadIntWithRetry("Enter Consultation Fee: ", "Enter a valid consultation fee.");
                if (doctor.Fees < 0)
                {
        ...
        private static Doctor.Specialisations GetSpecialisationFromUser()
        {
            while (true)
            {
                ...
                if (Enum.TryParse(specialisationInput, true, out Doctor.Specialisations result)
                    && Enum.IsDefined<Doctor.Specialisations>(result))
                {
                    return result;
                }
        ...
        private static Patient.Genders ReadGender()
        {
            while (true)
            {
                ...
                var input = Console.ReadLine();
                if (Enum.TryParse(input, true, out Patient.Genders gender) && Enum.IsDefined<Patient.Genders>(gender))
                {
                    return gender;
                }
        ...
        public void UpdatePatient()
        {
            try
            {
                ...
                patient.PatientName = ReadOptionalFullName(patient.PatientName)?? string.Empty;
        ...
        public void UpdateDoctor()
        {
            try
            {
                ...
                doctor.DoctorName = ReadOptionalFullName(doctor.DoctorName);
                doctor.Specialisation = ReadOptionalSpecialisation(doctor.Specialisation);
                doctor.Experience = ReadOptionalIntWithinRange(doctor.Experience, $"Experience ({doctor.Experience}): ", 0, 50);
                doctor.Fees = ReadOptionalNonNegativeInt(doctor.Fees, $"Fee ({doctor.Fees}): ");
