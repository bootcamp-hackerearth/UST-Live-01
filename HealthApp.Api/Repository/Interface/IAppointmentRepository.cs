...
namespace HealthApp.Api.Repository.Interface
{
    public interface IAppointmentRepository : IGenericRepository<Appointment>
    {
        ...
        Task<(List<Appointment> Items, int TotalCount)> GetByPatientAndDoctor(int? patientId,int? doctorId,
            int pageNumber,int pageSize);

        Task<(List<Appointment> Items, int TotalCount)> GetPagedAppointmentsAsync( int pageNumber, int pageSize);
