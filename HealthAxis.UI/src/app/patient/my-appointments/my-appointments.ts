...
export class MyAppointments {
  ...
  printAppointmentPdf(appointment: Appointment): void {
    if (!this.canDownloadAppointmentPdf(appointment)) {
      this.errorMessage.set('PDF is not available for cancelled appointments.');
      return;
    }
    ...
    const specialisation = this.escapeHtml(
      this.getSafeText(appointment.specialisation, 'Not assigned')
    );
    const appointmentDate = this.escapeHtml(
      new Date(appointment.scheduledDate).toLocaleDateString()
    );
    const timeSlot = this.escapeHtml(
      this.getSafeText(appointment.timeSlot, 'Not assigned')
    );
    const status = this.escapeHtml(appointment.status || 'Pending');
      ...
      <html>
        ...
        <body>
          <main class="document">
            ...
            <section class="note">
              Please check your appointment status before visiting the hospital.
              Completed appointments may have health records available in the Health Records section.
            </section>
      ...
      <html>
        ...
        <body>
          <main class="document">
            ...
            <section class="footer">
              This record is generated by HealthAxis. Please consult your doctor before changing any medication.
            </section>
          </main>
...
onHealthRecordBackdropClick(event: MouseEvent): void {
  ...
  private getAppointmentStartDateTime(appointment: Appointment): Date {
    ...
    const timeParts = appointment.timeSlot.trim().match(/^(\d{1,2}):(\d{2})\s*(AM|PM)$/i);
