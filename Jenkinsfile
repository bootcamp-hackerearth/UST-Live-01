pipeline {
    ...
    environment {
        ...
        EB_APPLICATION_NAME = 'HealthAxisApplicn'
        EB_ENVIRONMENT_NAME = 'HealthAxisApplicn-dev'
        S3_BUCKET = 'jenkins-sprint5-938463256613-ap-south-1-an'
    }
    ...
    stages {
        ...
        stage('Publish Blazor') {
            steps {
                bat 'dotnet publish HealthAxisAdminPortal\\HealthAxisAdminPortal.csproj -c Release -o blazor-publish-temp'
            }
        }

        stage('Copy Blazor into API wwwroot') {
            steps {
                bat 'if not exist HealthAxisApplicn\\wwwroot\\blazor mkdir HealthAxisApplicn\\wwwroot\\blazor'
                bat 'xcopy /E /Y /I blazor-publish-temp\\wwwroot\\* HealthAxisApplicn\\wwwroot\\blazor\\'
            }
        }

        stage('Publish API') {
            steps {
                bat 'dotnet publish HealthAxisApplicn\\HealthAxisApplicn.csproj -c Release -o publish'
            }
        }
