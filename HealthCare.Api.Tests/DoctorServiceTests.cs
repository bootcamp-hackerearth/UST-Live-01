...
namespace HealthCare.Tests
{
    ...
    public class DoctorServiceTests
    {
        ...
        public async Task AddDoctor_ShouldCreateDoctorAndSlots()
        {
            ...
            _repoMock.Setup(r => r.AddAsync(doctor)).Returns(Task.CompletedTask);
            ...
            Assert.IsNotNull(result);
            Assert.IsTrue(result.IsActive, "Doctor should be active");
            Assert.AreEqual(1, result.DoctorId);
        }
        ...
        {
            var existing = new Doctor { DoctorId = 1, FullName = "Old" };
        ...
        public async Task DeleteDoctor_ShouldSoftDelete()
        {
            ...
            _repoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(doctor);
            _repoMock.Setup(r => r.UpdateAsync(doctor)).Returns(Task.CompletedTask);
