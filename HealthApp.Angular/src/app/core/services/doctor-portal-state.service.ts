...
import { Appointment, AppointmentStatus, UpdateAppointmentStatusRequest } from '../models/appointment.model';
import { AddHealthRecordRequest, HealthRecord } from '../models/health-record.model';
...
export class DoctorPortalStateService {
  ...
  readonly todayAppointments = computed(() => {
    ...
    return this.appointments()
      .filter(appointment => this.toDateOnly(appointment.scheduledDate) === today)
  ...
  loadPatientContext(patientId: number): void {
    ...
    forkJoin({
      ...
      history: this.healthRecordApi.getHealthRecordsByPatient(patientId)
    }).subscribe({
      next: result => {
  ...
  loadRecordByAppointment(appointment: Appointment): void {
    this.healthRecordApi.getHealthRecordsByPatient(appointment.patientId).subscribe({
      next: records => {
        const record = records.find(item => item.appointmentId === appointment.appointmentId) ?? null;
  ...
  private updateAppointmentStatus(
    ...
    return this.appointmentApi.updateAppointmentStatus(appointmentId, request).pipe(
