    ...
    DateTime? scheduledDate)
        ...
        public async Task<ActionResult> HealthRecords(int? patientId)
        {
            try
            {
                ViewBag.PatientId = patientId.Value;
                ViewBag.Patient = await _patientApiService.GetByIdAsync(patientId.Value);
                ViewBag.Doctors = await _doctorApiService.GetAllAsync();
       ...
       Specialisation? specialisation)
        {
            try
            {
                ...
                ViewBag.SearchBy = string.IsNullOrWhiteSpace(searchBy) ? "All" : searchBy;
                ViewBag.SelectedSpecialisation = specialisation;

                List<DoctorDto> doctors = await _doctorApiService.GetAllActiveAsync();

                IEnumerable<DoctorDto> filteredDoctors = doctors;
                ...
                if (!string.IsNullOrWhiteSpace(searchText))
                {
                    ...
                    string selectedSearchBy = string.IsNullOrWhiteSpace(searchBy)
                        ? "All"
                        : searchBy;

                    if (selectedSearchBy == "Id")
                    {
                        filteredDoctors = filteredDoctors
                            .Where(d => isId && d.DoctorId == doctorId);
                    }
                    else if (selectedSearchBy == "Name")
                    {
                        filteredDoctors = filteredDoctors
                            .Where(d =>
                ...
                return View(filteredDoctors.ToList());
            }
            catch (Exception ex)
            {
                ViewBag.PatientId = patientId.Value;
