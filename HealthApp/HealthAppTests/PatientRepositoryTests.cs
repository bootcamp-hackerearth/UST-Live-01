...
namespace HealthAppTests
{
    public class PatientRepositoryTests
    {
        ...
        public PatientRepositoryTests()
        {
            ...
            //_patientDb.patient.AddRange(new List<Patient>
            //{
            //    new Patient                
            //    {
            //        PatientId = 1,
        ...
        public void GetAll_WhenCalled_ShouldReturn4Patients()
        {
            var patients = _patientRepository.GetAll();
            Assert.NotNull(patients);
            Assert.Equal(4, patients.Count);
            Assert.Equal("Mark", patients[0].FullName);
        }
...

        {
            ...
            Assert.Equal("Product with id 2 updated successfully", result);
            var patient = _patientRepository.GetById(2);
        ...
        public void Update_WhenProvidedWithInvalidId_ThrowsPatientNotFoundException()
        {
            ...
            Assert.Throws<PatientNotFoundException>(() => _patientRepository.UpdatePatient(10, patient));
        }
        ...
        public void Delete_WhenProvidedWithPatientId_DeletePatient()
        {
            ...
            Assert.NotNull(result);
            Assert.Equal("Product with id 3 deleted successfully", result);
            Assert.Throws<PatientNotFoundException>(() => _patientRepository.GetById(3));
        }
        [Fact]
        public void Delete_WhenProvidedWithInvalidId_ThrowsPatientNotFoundException()
        {
            Assert.Throws<PatientNotFoundException>(() => _patientRepository.DeletePatient(100));
        }

        [Fact]
        public void GetById_WhenProvidedInvalidId_ThrowsPatientNotFoundException()
        {
            Assert.Throws<PatientNotFoundException>(() => _patientRepository.GetById(50));
        }
    }
}
